...
def write_doc(data, fname):
    doc = docx.Document() # open an existing document
...
def CP_data(N, text_list):
    ...
    data_dict['rho'] = text_list[i+24].strip().split()[-1]
    data_dict['Lap'] = text_list[i+29].strip().split()[-1]
    data_dict['V'] = text_list[i+46].strip().split()[-1]
    data_dict['G'] = text_list[i+45].strip().split()[-1]
    data_dict['H'] = text_list[i+47].strip().split()[-1]
    data_dict['M'] = text_list[i+32].strip().split()[-1]
    data_dict['elip'] = text_list[i+33].strip().split()[-1]
    return data_dict
